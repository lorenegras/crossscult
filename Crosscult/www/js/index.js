...
var cordovaExample = {
  ...
  //on définit l'id du visiteur avec un nombre aléatoire de 6 chiffres 
  idClient : Math.floor((Math.random() * 10000000) + 1),
  //sert si on a besoin d'un compteur (voir plus bas dans la fonction showLocation)
  compteur : 0,
  ...
  showLocation: function(position) {
    ...
    try {
	  ...
	  //Pour envoyer la position du visiteur à la BDD FireBase
	  // Normalement vous êtes reliés au projet Crosscult de FireBase avec l'adresse suivante : equipe.kiwi@gmail.com
	  /* Si jamais il y a trop d'informations envoyées à la BDD 
	  (saturation de la BDD car chaque mvt possède une ligne) 
	  mettre un compteur et l'adapter en fonction de ce qui est envoyé*/
	  /*this.compteur = this.compteur + 1;
	  if (this.compteur > 100){ */
	  // on définit chaque variable
		  var id = cordovaExample.idClient;
		  var temps = Math.floor(Date.now() / 1000);
		  var latitude = position.coords.latitude;
		  var longitude = position.coords.longitude;
		// on choisit d'afficher dans la base de données l'id du visiteur, le temps, la lattitude et la longitude qui représente la position du visiteur à un instant t 
		  param = id+";"+temps+";"+latitude+";"+longitude;
		 jQuery.ajax({
			 // envoie cette information à FireBase
	     	 url: "https://crosscult-de6dc.firebaseio.com/positions.json",
			 type: "POST",
			 dataType: "json",
  ...
  initializeMap: function() {
    ...
    var mapProp = {
      center: new google.maps.LatLng(48.694915, 6.188174),
      zoom: 15,
